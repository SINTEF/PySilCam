    ...
    if args['process']:
        ...
        if nbImages is not None:
            try:
                ...
                print('__main__ Expected type int for --nbimages.')
                sys.exit(0)
        if args['--appendstats']:
        ...
        try:
            ...
            print('__main__ Could not prioritise acquisition process!')
        ...
        if args['--discread']:
            ...
            print('__main__ discWrite = False')
            discWrite = False
...
def silcam_process(config_filename, datapath, multiProcess=True, realtime=False,
    ...
    print('__main__  realtime', realtime)
    ...
    if os.path.isfile(datafile_hdf):
        ...
        if overwriteSTATS:
            ...
            print('__main__ Overwriting ' + datafile_hdf)
    ...
    print('__main__ * Commencing image acquisition and processing')
    ...
    distributor(proc_image_queue, outputQueue, config_filename, proc_list, gui)
    ...
    #    print('__main__ acq = Acquire(USE_PYMBA=False)')
    ...
    logger.info('%s.exitcode = %s' % (bg_process.name, bg_process.exitcode))

    print('__main__ PROCESSING COMPLETE.')
...
def distributor(proc_image_queue, outputQueue, config_filename, proc_list, gui=None):
    ...
    print('__main__ forcing processing cores to 1 only!')
    numCores = 1
    ...
    if path:
        if not os.path.isdir(path):
            try:
                ...
                print('__main__ Could not create catalog:', path)
...
def update_pysilcam_offset(logger, settings, datafilename, datapath):
    ...
    print('__main__ Loading old data from: ' + datafile_hdf)
    ...
    print('__main__ Calculating spooling offset')
    ...
    print('__main__ offset set to: ' + offset)
    offset = int(offset)
