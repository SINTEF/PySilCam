...
def silcam_process(config_filename, datapath, multiProcess=True, realtime=False, discWrite=False, nbImages=None, gui=None,
    ...
    if os.path.isfile(datafilename + '-STATS.csv') and overwriteSTATS:
        ...
        files = [f for f in sorted(os.listdir('../SilCam/LowMag/'))
                 if f.endswith('.silc')]
    ...
    if (multiProcess):
        bgstack, imbg = scbg.ini_background(settings.Background.num_images, aqgen)
        ...
        logger.debug('max distributor queue size: ' + str(distributor_q_size))
        ...
        distributor(inputQueue, outputQueue, config_filename, proc_list, bgstack, gui)
        ...
        for i, (timestamp, imraw) in enumerate(aqgen):
            ...
            addToQueue(realtime, inputQueue, i, timestamp, imraw) # the tuple (i, timestamp, imc) is added to the inputQueue
...
def loop(config_filename, inputQueue, outputQueue, bgstack, gui=None):
    '''
    ...
    #bgstack = []
    #task = inputQueue.get()
    #bgstack.append(task[2]) # get the first image
    #for i in range(settings.Background.num_images - 1): # loop through the rest, appending to bgstack
    #    task = inputQueue.get()
    #    bgstack.append(task[2])
    ...
    while True:
        ...
        if task is None:
            ...
            break
        # shift and correct background
        bgstack, imbg, imc = scbg.shift_and_correct(bgstack, imbg, task[2],
                                               settings.Background.num_images, real_time_stats=settings.Process.real_time_stats)

        task2 = (task[0], task[1], imc)

        # update task to contain imc instead of imraw
        stats_all = processImage(nnmodel, class_labels, task2, settings, logger, gui)
...
def distributor(inputQueue, outputQueue, config_filename, proc_list, bgstack, gui=None):
    '''
